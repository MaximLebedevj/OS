...
#ifdef _WIN32
    snprintf(log_message, sizeof(log_message), "[%s] copy2   pid %lu, STARTED\n", curr_time, GetPid());
    WriteLog(log_file, log_message);
#else
    printf("[%s] copy2   pid %d, STARTED\n", curr_time, GetPid());
#endif
...
#ifdef _WIN32
    snprintf(log_message, sizeof(log_message), "[%s] copy2   pid %lu, EXITED\n", curr_time, GetPid());
    WriteLog(log_file, log_message);
#else
    printf("[%s] copy2   pid %d, EXITED\n", curr_time, GetPid());
#endif
