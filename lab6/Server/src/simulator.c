...
int main(int argc, char *argv[])
{
    ...
    if (!configure_port(hSerial, BAUDRATE_115200)) {
        perror("configure_port (hSerial)");
        CloseHandle(hSerial);
        exit(EXIT_FAILURE);
    }
    #else
    ...
    configure_port(port_wr, baud_rate);
    ...
    // generate random initial temp
    double temp = init_rand_temp(-50, 50);
    char data[10];
